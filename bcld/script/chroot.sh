...
# Keep APT's normal archive directory only when the builder mounted the
# repository-local package cache there. The cache mount is removed before the
# final SquashFS is generated.
function clean_apt_archives () {
    local APT_CLEAN_DIR

    if [[ "${BCLD_USE_CACHE:-0}" != '0' ]]; then
        list_item 'BCLD package cache: retaining downloaded archives'
        APT_CLEAN_DIR="$(/usr/bin/mktemp --directory --suffix=_BCLD-APT-CLEAN)"
        /usr/bin/mkdir -p "${APT_CLEAN_DIR}/partial"
        /usr/bin/apt-get -o Dir::Cache::archives="${APT_CLEAN_DIR}" clean
        /usr/bin/rm -rf "${APT_CLEAN_DIR}"
    else
