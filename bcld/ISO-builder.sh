...
APT_CACHE_DIR="${PROJECT_DIR}/.build-cache/apt/archives"
...
CHROOT_APT_ARCHIVES="${CHROOT_DIR}/var/cache/apt/archives"
...
export BCLD_USE_CACHE="${BCLD_USE_CACHE:-1}"
...
## Function to prepare the optional, build-only APT archive cache
function prepare_apt_cache () {
    if [[ "${BCLD_USE_CACHE}" == '0' ]]; then
        list_item 'BCLD package cache: disabled'
        return
    fi

    /usr/bin/mkdir -p "${APT_CACHE_DIR}/partial"
    /usr/bin/chmod 755 "${APT_CACHE_DIR}"
    list_item 'BCLD package cache: enabled'
    list_item 'BCLD package cache: .build-cache/apt/archives'
    list_item "BCLD cached packages available: $(/usr/bin/find "${APT_CACHE_DIR}" -maxdepth 1 -type f -name '*.deb' | /usr/bin/wc -l)"
}
...
function init_mount () {
    ...
    if [[ "${BCLD_USE_CACHE}" != '0' ]]; then
        /usr/bin/mkdir -p "${CHROOT_APT_ARCHIVES}/partial" "${APT_CACHE_DIR}/partial"
        /usr/bin/chown "$(/usr/bin/stat -c '%u:%g' "${CHROOT_APT_ARCHIVES}/partial")" "${APT_CACHE_DIR}/partial"
        /usr/bin/chmod "$(/usr/bin/stat -c '%a' "${CHROOT_APT_ARCHIVES}/partial")" "${APT_CACHE_DIR}/partial"
        /usr/bin/mount --verbose --bind "${APT_CACHE_DIR}" "${CHROOT_APT_ARCHIVES}"
    fi
    list_catch
}
...
function unmount_chroot () {
    ...
    clear_mount "${CHROOT_APT_ARCHIVES}"
...
DEBOOTSTRAP_CACHE_ARGS=()
if [[ "${BCLD_USE_CACHE}" != '0' ]]; then
    DEBOOTSTRAP_CACHE_ARGS=(--cache-dir="${APT_CACHE_DIR}")
fi
...
if [[ -f /usr/sbin/debootstrap ]] && [[ ! -d "${BOOTSTRAP_DIR}" ]]; then
    ...
    list_entry
    /usr/sbin/debootstrap "${DEBOOTSTRAP_CACHE_ARGS[@]}" --variant=minbase --arch=amd64 "${CODE_NAME}" "${BOOTSTRAP_DIR}" "${UBUNTU_REPO}" | /usr/bin/tee "${BOOTSTRAP_LOG}"
    list_catch
...
if [[ "${BCLD_USE_CACHE}" != '0' ]]; then
    list_item "BCLD cached packages after install: $(/usr/bin/find "${APT_CACHE_DIR}" -maxdepth 1 -type f -name '*.deb' | /usr/bin/wc -l)"
fi
...
# 4TW customization: let PID 1 handle Ctrl+Alt+Delete as a clean poweroff.
list_item "Configuring Ctrl+Alt+Delete to power off..."
/usr/bin/ln -sfn /usr/lib/systemd/system/poweroff.target "${CHSERVICE_DIR}/ctrl-alt-del.target"
